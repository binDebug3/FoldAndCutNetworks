...
from BenchmarkTests.RL.nf.nets import MLP
from BenchmarkTests.RL.nf.transforms import Preprocessing
from BenchmarkTests.RL.nf.distributions import ConditionalDiagLinearGaussian
from BenchmarkTests.RL.nf.flows import MaskedCondAffineFlow, CondScaling
...
def init_Flow(sigma_max, sigma_min, action_sizes, state_sizes, fold):
    init_parameter = "zero"
...
class FlowPolicy(nn.Module):
    def __init__(self, alpha, sigma_max, sigma_min, action_sizes, state_sizes, fold, device):
        super().__init__()
        self.device = device
        self.alpha = alpha
        self.action_shape = action_sizes
        flows, q0 = init_Flow(sigma_max, sigma_min, action_sizes, state_sizes, fold)
